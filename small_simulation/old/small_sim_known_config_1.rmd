...
likelihood_alpha_current = compute_likelihood(arm_hist, Y_hist, alpha_current, A) #likelihood at alpha_current
likelihood_alpha_new = compute_likelihood(arm_hist, Y_hist, alpha_new, A) #likelihood at alpha_new
...
sample_alpha_current = dgamma(alpha_current_d, shape = alpha_new_d*tuning, rate = tuning)  # g(alpha_current_d|alpha_new_d) Gamma(alpha_new_d*tuning,tuning)
sample_alpha_new = dgamma(alpha_new_d, shape = alpha_current_d*tuning, rate = tuning) # g(alpha_new_d|alpha_current_d) Gamma(alpha_current_d*tuning,tuning)

r = (likelihood_alpha_new*prior_alpha_new*sample_alpha_current)/
  (likelihood_alpha_current*prior_alpha_current*sample_alpha_new)
```
...
```{r}
mean(rfrechet(10000,shape = alpha[2]))
```


Try to infer alpha based on samples

Experiment 1: try arm 1 10 times and arm 2 10 times
```{r}
# play 1st arm 10 times and then 2nd arm 10 times
arm_hist = c(rep(1,10), rep(2,10))
...
tuning = 0.5 # tuning parameter of the proposal distribution
burnin = 1000
size = 10000 # total number of posterior samples before thining
thining = 5
...
- check tuning parameter for Metropolis
- change likelihood to log likelihood
- simulate (Thursday)
* * *
