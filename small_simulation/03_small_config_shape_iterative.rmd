...
Check that the algorithm to sample new configuration works in the ideal scenario 
```{r}
# sample code to update A using alpha_current
...
alpha_current = alpha_truth
PROB = matrix(0, nrow = K, ncol = D)
...
# The algorithm can infer the correct configuration given enough data and the correct shape parameter
print(A_current)
print(PROB)
```
